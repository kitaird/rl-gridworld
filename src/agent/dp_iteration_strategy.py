...
class DpIterationStrategy(IterationStrategy):
    ...
    #
    # def action_value(self, state, action):
    #     new_state, action_value = self._env.get_new_state_and_reward(state, action)
    #     return action_value + self.discount_factor() * self._state_values[new_state]
    #
    # def policy_evaluation(self):
    #     small_enough = 1e-3
    #     while True:
    ...
    #                 if state in self._policy:
    #                     old_action = self._policy[state]
    #                     self._state_values[state] = self.action_value(state, old_action)
    #                     biggest_change = max(biggest_change, np.abs(old_state_value - self._state_values[state]))
